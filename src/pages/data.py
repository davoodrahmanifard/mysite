...
def main():
    ...
    h_recovered = df.groupby("Country_Region").agg({"Recovered": "sum"}).nlargest(2,"Recovered")
    st.markdown("* **{}** has the largest number of recoveries with **{:.2f}K** recovered.".format(h_recovered.index.values[0],
                                                                                           h_recovered["Recovered"].values[0]))
